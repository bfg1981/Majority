import { getManifest } from "./getManifest.js";

const idSelect = document.getElementById("governing-body-id");
const periodSelect = document.getElementById("governing-body-period");
const outputId = "governing-body-output";
const storageKeys = {
  governingBodyId: "majority.governingBodyId",
  governingBodyPeriod: "majority.governingBodyPeriod",
  deploymentVersion: "majority.deploymentVersion",
};

// Settings UI (introspected from window.GB_CONFIG, defined in governing-body-viewer.js)
const gearBtn = document.getElementById("gb-settings-gear");
const settingsPanel = document.getElementById("gb-settings-panel");
const settingsBody = document.getElementById("gb-settings-body");
const settingsClose = document.getElementById("gb-settings-close");

function clampNumber(n, min, max) {
  if (typeof min === "number" && Number.isFinite(min)) n = Math.max(min, n);
  if (typeof max === "number" && Number.isFinite(max)) n = Math.min(max, n);
  return n;
}

function buildControlRow(key, entry) {
  const row = document.createElement("div");
  row.className = "gb-setting-row";

  const label = document.createElement("label");
  label.className = "gb-setting-label";
  label.textContent = entry.label || key;

  const controlWrap = document.createElement("div");
  controlWrap.className = "gb-setting-control";

  const valueType = typeof entry.value;

  let input;
  if (valueType === "number") {
    input = document.createElement("input");
    input.type = "number";
    if (typeof entry.min === "number") input.min = String(entry.min);
    if (typeof entry.max === "number") input.max = String(entry.max);
    if (typeof entry.step === "number") input.step = String(entry.step);
    input.value = String(entry.value);
    input.addEventListener("change", () => {
      const parsed = Number(input.value);
      if (!Number.isFinite(parsed)) return;
      entry.value = clampNumber(parsed, entry.min, entry.max);
      input.value = String(entry.value);
    });
  } else if (valueType === "boolean") {
    input = document.createElement("input");
    input.type = "checkbox";
    input.checked = Boolean(entry.value);
    input.addEventListener("change", () => {
      entry.value = Boolean(input.checked);
    });
  } else {
    input = document.createElement("input");
    input.type = "text";
    input.value = String(entry.value ?? "");
    input.addEventListener("change", () => {
      entry.value = input.value;
    });
  }

  input.className = "gb-setting-input";
  const id = `gb-setting-${key}`;
  input.id = id;
  label.htmlFor = id;

  controlWrap.appendChild(input);
  row.appendChild(label);
  row.appendChild(controlWrap);

  return row;
}

function setupSettingsUI() {
  if (!gearBtn || !settingsPanel || !settingsBody || !settingsClose) return;
  const cfg = window.GB_CONFIG;
  if (!cfg || typeof cfg !== "object") return;

  // Build controls once from current CONFIG keys.
  settingsBody.innerHTML = "";
  for (const [key, entry] of Object.entries(cfg)) {
    if (!entry || typeof entry !== "object" || !("value" in entry)) continue;
    settingsBody.appendChild(buildControlRow(key, entry));
  }

  function openPanel() {
    settingsPanel.hidden = false;
    gearBtn.setAttribute("aria-expanded", "true");
  }

  function closePanel() {
    settingsPanel.hidden = true;
    gearBtn.setAttribute("aria-expanded", "false");
  }

  function togglePanel() {
    if (settingsPanel.hidden) openPanel();
    else closePanel();
  }

  gearBtn.addEventListener("click", togglePanel);
  settingsClose.addEventListener("click", closePanel);

  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && !settingsPanel.hidden) closePanel();
  });

  document.addEventListener("click", (e) => {
    if (settingsPanel.hidden) return;
    const t = e.target;
    if (t === gearBtn) return;
    if (settingsPanel.contains(t)) return;
    closePanel();
  });
}

function readStoredValue(key) {
  try {
    return localStorage.getItem(key);
  } catch {
    return null;
  }
}

function writeStoredValue(key, value) {
  try {
    localStorage.setItem(key, value);
  } catch {
    // Ignore storage errors; selection persistence is optional.
  }
}

function removeStoredValue(key) {
  try {
    localStorage.removeItem(key);
  } catch {
    // Ignore storage errors; selection persistence is optional.
  }
}

function invalidateStoredSelectionIfVersionChanged(deploymentVersion) {
  const currentVersion =
    typeof deploymentVersion === "string" && deploymentVersion
      ? deploymentVersion
      : null;
  const storedVersion = readStoredValue(storageKeys.deploymentVersion);

  if (storedVersion !== currentVersion) {
    removeStoredValue(storageKeys.governingBodyId);
    removeStoredValue(storageKeys.governingBodyPeriod);
  }

  if (currentVersion) {
    writeStoredValue(storageKeys.deploymentVersion, currentVersion);
  } else {
    removeStoredValue(storageKeys.deploymentVersion);
  }
}

const domReady = new Promise((resolve) => {
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", resolve, { once: true });
  } else {
    resolve();
  }
});

window.DEPLOYMENT_VERSION = null;
window.DEV_MODE = false;

function normalizeDeploymentInfo(data) {
  const version =
    data && typeof data.version === "string" && data.version
      ? data.version
      : null;
  return {
    version,
    devMode: Boolean(data && data.devMode),
  };
}

function withDevCacheBust(url, deploymentInfo) {
  if (!deploymentInfo.devMode || !deploymentInfo.version) return url;
  const u = new URL(url, window.location.href);
  u.searchParams.set("dev", deploymentInfo.version);
  return u.pathname + u.search + u.hash;
}

const deploymentVersionReady = fetch("/deployment-version.json", {
  headers: { Accept: "application/json,*/*;q=0.8" },
})
  .then((res) => {
    if (!res.ok) return null;
    return res.json();
  })
  .then(normalizeDeploymentInfo)
  .catch(() => normalizeDeploymentInfo(null))
  .then((deploymentInfo) => {
    window.DEPLOYMENT_VERSION = deploymentInfo.version;
    window.DEV_MODE = deploymentInfo.devMode;
    return deploymentInfo;
  });

function loadManifest(deploymentInfo) {
  return new Promise((resolve) => {
    getManifest(resolve, {
      cachedManifestUrl: withDevCacheBust("/config/manifest.json", deploymentInfo),
      mapConfigUrl: (url) => withDevCacheBust(url, deploymentInfo),
      slowOptions: {
        discoveryOptions: {
          manifestUrl: withDevCacheBust("/config/index.json", deploymentInfo),
          autoindexUrl: withDevCacheBust("/config/", deploymentInfo),
        },
      },
    });
  });
}

const manifestReady = deploymentVersionReady
  .then(loadManifest)
  .catch(() => null)
  .then((manifest) => manifest || {});

Promise.all([domReady, manifestReady, deploymentVersionReady]).then(([, manifest, deploymentInfo]) => {
  setupSettingsUI();
  invalidateStoredSelectionIfVersionChanged(deploymentInfo.version);
  const ids = Object.keys(manifest).sort((a, b) => {
    const aIsMock = a.startsWith("X-");
    const bIsMock = b.startsWith("X-");
    if (aIsMock !== bIsMock) return aIsMock ? 1 : -1;
    return a.localeCompare(b);
  });
  if (ids.length === 0) return;

  // Populate ID dropdown
  idSelect.innerHTML = "";
  for (const id of ids) {
    const opt = document.createElement("option");
    opt.value = id;
    opt.textContent = id;
    idSelect.appendChild(opt);
  }

  //Disable if only one id
  idSelect.disabled = ids.length <= 1;

  const storedId = readStoredValue(storageKeys.governingBodyId);
  const storedPeriod = readStoredValue(storageKeys.governingBodyPeriod);
  let currentId = storedId && ids.includes(storedId) ? storedId : ids[0];
  let currentPeriod = storedPeriod;
  idSelect.value = currentId;

  function storeCurrentSelection() {
    if (currentId) writeStoredValue(storageKeys.governingBodyId, currentId);
    if (currentPeriod) writeStoredValue(storageKeys.governingBodyPeriod, currentPeriod);
  }

  function populatePeriods(preservePeriod = true) {
    const periods = Object.keys(manifest[currentId] || {});
    periodSelect.innerHTML = "";

    for (const p of periods) {
      const opt = document.createElement("option");
      opt.value = p;
      opt.textContent = p;
      periodSelect.appendChild(opt);
    }

    if (preservePeriod && currentPeriod && periods.includes(currentPeriod)) {
      periodSelect.value = currentPeriod;
    } else {
      currentPeriod = periods[periods.length - 1] || null;
      periodSelect.value = currentPeriod;
    }

    //Disable if only one id
    periodSelect.disabled = periods.length <= 1;
  }

  function loadCurrent() {
    if (!currentId || !currentPeriod) return;
    const file = manifest[currentId][currentPeriod];
    if (file) {
      loadGoverningBody(withDevCacheBust(file, deploymentInfo), outputId);
    }
  }

  // Initial population + load
  populatePeriods(true);
  storeCurrentSelection();
  loadCurrent();

  idSelect.addEventListener("change", () => {
    const previousPeriod = currentPeriod;
    currentId = idSelect.value;
    currentPeriod = previousPeriod;
    populatePeriods(true);
    storeCurrentSelection();
    loadCurrent();
  });

  periodSelect.addEventListener("change", () => {
    currentPeriod = periodSelect.value;
    storeCurrentSelection();
    loadCurrent();
  });
});
